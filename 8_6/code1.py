...
class MdmAccountCalendar(models.Model):
    ...
    def _check_uniqueness_check(self):
        ...
        for record in self:
            if self.search_count([('number', '=', record.number), ("delete_state", '=', "normal")]) > 1:
                self.biz_logs.append(BizLogItem(cls=record, operation=_('Save'), checked_type=_("Unique"),  message=_('Number must be unique!'),  level=LogLevel.ERROR, category=LogCategory.CHECK))
                raise ValidationError(_("Number already exists."))
            if self.search_count([('name', '=', record.name), ("delete_state", '=', "normal")]) > 1:
                self.biz_logs.append(BizLogItem(cls=record, operation=_('Save'),  checked_type=_("Unique"), message=_('Name must be unique!'),  level=LogLevel.ERROR, category=LogCategory.CHECK))
                raise ValidationError(_("Name already exists."))
            self.biz_logs.append(BizLogItem(cls=record,operation=_('Save'), checked_type=_("Unique"), message=_('[Number],[Name] unique check success!'),level=LogLevel.INFO,category=LogCategory.CHECK))
    ...
    def _check_calendar_date(self):
        ...
        if len(self.start_fiscal_year) != 4:
            self.biz_logs.append(BizLogItem(cls=self, operation=_('Save'), checked_type=_("Mismatch"),  message=_('Fiscal year is not correct!'),  level=LogLevel.ERROR, category=LogCategory.CHECK))
            raise ValidationError(_('The fiscal year format is incorrect!')) # 会计年度格式不正确
        if int(self.start_date.day) >= 28:
            self.biz_logs.append(BizLogItem(cls=self, operation=_('Save'), checked_type=_("Mismatch"),  message=_('Start date can not later than 28!'),  level=LogLevel.ERROR, category=LogCategory.CHECK))
            raise ValidationError(_("date should not be later 28!"))
        if self.start_fiscal_year[:4] != str(self.start_date)[:4]:
            self.biz_logs.append(BizLogItem(cls=self, operation=_('Save'),  checked_type=_("Mismatch"), message=_('Start Year must match with accounting year.'),  level=LogLevel.ERROR, category=LogCategory.CHECK))
            raise ValidationError(_("The start year and the start date must belong to the same accounting year."))
        self.biz_logs.append(BizLogItem(cls=self, operation=_('Save'), checked_type=_("Mismatch"), message=_('[Start year],[Start date] match check success!'), level=LogLevel.ERROR,category=LogCategory.CHECK))
        ...
        for res in self:
            ...
            if res.line_ids:  
                if (res.state == 'save') and (res.forbid_state == 'normal'):
                    ...
                    self.biz_logs.append(BizLogItem(cls=res, operation=_('Submit'),checked_type=_("Mismatch"), message=_('Submit Success!'), level=LogLevel.INFO, category=LogCategory.CHECK))
                    self.biz_logs.append(BizLogItem(cls=res, operation=_('Submit'), message=_('Submit Success!'), level=LogLevel.INFO, category=LogCategory.OPERATE))
                    line_info = LineInfo(operation=operation, state=SUCCESS_STATE, info=info)  
                else:
                    info = _('%s failed to submit because it is not on save and unforbidden state!') % res.display_name  
                    self.biz_logs.append(BizLogItem(cls=res, operation=_('Submit'),checked_type=_("Mismatch"), message=_('State is not match!'), level=LogLevel.ERROR, category=LogCategory.CHECK))
                    self.biz_logs.append(BizLogItem(cls=res, operation=_('Submit'), message=_('State is not match!'), level=LogLevel.ERROR, category=LogCategory.OPERATE))
                    line_info = LineInfo(operation=operation, state=FAILED_STATE, info=info)    
            else:
                info = _('%s faild to Submit ,because it has no accounting periods.') % res.display_name
                self.biz_logs.append(BizLogItem(cls=res, operation=_('Periods Check'),checked_type=_("Mismatch"), message=_('Please Add accounting periods.'), level=LogLevel.ERROR, category=LogCategory.CHECK))
                self.biz_logs.append(BizLogItem(cls=res, operation=_('Submit'), message=_('State is not match!'), level=LogLevel.ERROR, category=LogCategory.OPERATE))
                line_info = LineInfo(operation=operation, state=FAILED_STATE, info=info)  
    ...
    def create_periods(self):
        ...
        self._create_periods(start_date, self.additional, additional_state=False)
    ...
    def _create_week(self, start_date, init_year, acct_period, additional_state, w_type, line_ids,record_id):
        ...
        while weeks > 0:
            ...
            if acct_week % 13 == 0 and acct_week % 52 != 0:  # 13,26,39,65%52(下一年的13周)...计算会计季度
                quarter = acct_week % 52 // 13
            elif acct_week % 52 == 0 or acct_week == 53:  # 52,104(下一年的52周)...计算会计季度
                quarter = 4
            ...
            max_calendar_year = self.env['mdm.account.calendar.line1'].search([('parent_id','=',record_id)],order='acct_year desc',limit=1).acct_year
    ...
    def _create_periods(self, start_date, account_year, **additional_state):
        ...
        if additional_state:
            acct_period = max([int(i.acct_period) for i in self.line_ids.filtered(lambda x: not x.is_adjustment)] if self.line_ids else [0])
        else:
            if self.line_ids:
                self.line_ids.unlink()
            acct_period = max([int(i.acct_period) for i in self.line_ids] if self.line_ids else [0])  # 获取最大会计期间
       
        if self.period_type == 'month':  # 期间类型为月份的创建期间逻辑
            self.end_date = start_date + relativedelta(years=account_year, days=-1)  # 计算结束日期
            month_index = 0
            while start_date < self.end_date:  # 循环产生12个期间
                ...
                acct_period += 1
                if acct_period % 3 == 0 and acct_period % 12 != 0:  # 03,06,09,15%12(下一年的03期间)...计算会计季度
                    quarter = acct_period % 12 // 3
                elif acct_period % 12 == 0:  # 12,24(下一年的12)...计算会计季度
                    quarter = 4
                elif acct_period % 3 != 0:  # 01,02,04,05,...,13%12(下一年的01期间)...计算会计季度
            ...
            if self.week_type == '52':  # 一年52周的计算方法
                w_type = 52
                self._create_week(start_date, init_year, acct_period, additional_state, w_type, line_ids,self.id)
                ...
                add_acct_year = init_year
                max_calendar_year = self.env['mdm.account.calendar.line1'].search([('parent_id','=',self.id)],order='acct_year desc',limit=1).acct_year
                if max_calendar_year and additional_state:
                    add_acct_year = int(max_calendar_year)+1
    ...
    def additional_peroids(self):
        ...
        if self.end_date:
            ...
            self.end_date = self.end_date + relativedelta(years=self.additional, days=-1)
            self._create_periods(start_date, self.additional, additional_state=True)
        else:
            self.biz_logs.append(BizLogItem(cls=self, operation=_('Add Periods'),checked_type=_("Mismatch"), message=_('End Date Error!'), level=LogLevel.ERROR, category=LogCategory.CHECK))
            self.biz_logs.append(BizLogItem(cls=self, operation=_('Add Periods'), message=_('End Date Error!'), level=LogLevel.ERROR, category=LogCategory.OPERATE))
    ...
    def p_get_next_period_by_date_calendar(self, biz_date, calendar_id):
        ...
        '''
        next_period = self.env['mdm.account.calendar.line1'].search([('parent_id','=',calendar_id.id),('is_adjustment','=',False),('start_date','>',biz_date)], order='start_date',limit=1)
    ...
    def unlink(self):
        ...
        for record in self:
            if record.state in ['temporary','save']:
                ...
                raise ValidationError(_("The period is not on temporary or save state"))
