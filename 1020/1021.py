...
class CmCashIncomeExpense(models.Model):
    ...
    def _get_line_dict(self, doc_ids, model, domain, fields):
        lines = self.env[model].search([('parent_id', 'in', doc_ids)] + domain)
        lines_data = lines.query_read(fields=fields)
    ...
    def _query_cm_receivable_bill_line1_sale_data(self, records, parent_model, child_model, cash_sett_type_ids):
        """
    ...
    def _query_cm_cash_recycling_line_data(self, records, parent_model, child_model, cash_sett_type_ids):
        """
        @desc: 创建现金存款单相关数据到后台表中
        @params: self: 实列
        @params records: 表头对象
        ...
        @return 返回数据列表
        """
        recycling_data = records.query_read( 
                fields=['number', 'create_uid', 'recv_org_id', 'biz_date', 'local_currency_id', 'state', 'document_type'])
        lines_dict = self._get_line_dict(
                doc_ids=records.ids, 
                domain=[],
                model=child_model,
                fields=['currency_id', 'sequence', 'cash_acct_number', 'pay_amount', 'pay_amount_lc', 'abstract', 'parent_id']
                )
            ...
            for line in lines_dict[parent['id']]:
                ...
                if amount != 0 or amount_local != 0:
                    ...
                    new_dict = {
                        ...
                        'cash_acct_number_id': line['cash_acct_number'], # 现金账号#
                        **parent_dict
                    }
                    # 存款单
                    ...
                    # 取款单
                    elif document_type == 'cash_withdrawals':
                        new_dict.update({
    ...
    def _query_cm_transfer_order_data(self, records, parent_model, child_model, cash_sett_type_ids):
        """
        @desc: 创建现金转账单相关数据到后台表中
        @params records: 表头对象
        @params parent_model: 表头模型名称
        ...
        parent_data = records.query_read( 
                fields=['number', 'create_uid', 'col_org_id', 'biz_date', 'local_curr_id', 'state'])
        lines_dict = self._get_line_dict(
                    doc_ids=records.ids, 
                    model=child_model,
                    domain=[],
                    fields=['currency_id', 'sequence', 'cash_out_account_id', 'in_amount', 'in_amount_lc', 'cash_in_account_id', 
                        'abstract', 'parent_id']
                    )
        result = []
        for parent in parent_data:
            ...
            parent_dict = {
                'source_create_uid': parent['create_uid'],  # 创建人#
                'number': parent['number'],  # 单据编号#
                'recpay_org_id': parent['col_org_id'],  # 收付组织#
                'biz_date': parent['biz_date'],  # 业务日期#
                'state': parent['state'],  # 单据状态#
                'local_currency_id': parent['local_curr_id'],  # 本位币#
                'source_parent_doc_id': parent['id'],  # 来源单据主表id
            ...
            for line in lines_dict[parent['id']]:
                ...
                if income_amount != 0 or income_amount_local != 0:
                    ...
                    new_dict = {
                        ...
                        'income_amount_local': income_amount_local,  # 收入金额本位币
                        'expense_amount_local': 0,  # 支出金额本位币
                        'cash_acct_number_id': line['cash_in_account_id'],  # 现金账号（转入）
                        **base_dict
                    }
                    result.append(new_dict)
                ...
                if income_amount != 0 or income_amount_local != 0:
                    ...
                    new_dict = {
                        ...
                        'income_amount_local': 0,  # 收入金额本位币
                        'expense_amount_local': income_amount_local,  # 支出金额本位币
                        'cash_acct_number_id': line['cash_out_account_id'],  # 现金账号（转出）
                        **base_dict
    ...
    def _query_cm_foreign_exchange_data(self, records, parent_model, child_model, cash_sett_type_ids):
        """
        @desc: 创建现金购汇单相关数据到后台表中
        @params: self: 实列
        @params records: 表头对象
        ...
        lines_dict = self._get_line_dict(
                ...
                fields=['out_currency_id', 'in_currency_id', 'sequence', 'cash_out_account_id', 'cash_in_account_id', 'out_amount', 'out_amount_lc', 
                    'in_amount', 'in_amount_lc', 'abstract', 'parent_id']
                )
        result = []
        for parent in parent_data:
            obj_local_currency_id = parent['local_curr_id']  # 本位币
    ...
    def _query_cm_receivable_refund_bill_line1_sale_data(self, records, parent_model, child_model, cash_sett_type_ids):
        ...
        """
        parent_data = records.query_read(
            fields=['number', 'create_uid', 'recv_org_id', 'biz_date', 'currency_id', 'sett_org_id', 'local_curr_id', 'state'])
        lines_dict = self._get_line_dict(
            doc_ids=records.ids, 
            model=child_model,
            domain=[('sett_type_id', 'in', cash_sett_type_ids)],
        ...
        for parent in parent_data:
            ...
            parent_dict = {
                ...
                'state': parent['state'],  # 单据状态#
                'local_currency_id': parent['local_curr_id'],  # 本位币#
                'sett_org_id': parent['sett_org_id'],  # 结算组织
                'currency_id': parent['currency_id'],  # 币别
                'local_currency_id': parent['local_curr_id'],  # 本位币
                'source_parent_doc_id': parent['id'],  # 来源单据主表id
                'source_parent_model': parent_model,  # 来源单据主表模型
    ...
    def _query_cm_payable_bill_line1_data(self, records, parent_model, child_model, cash_sett_type_ids):
        """
        @desc: 按照后台表格式，组织采购业务付款单/其他业务付款单相关数据，主要提供给查询和模拟审核使用
        @params: self: 实列
        @params records: 表头对象
        @params parent_model: 表头模型名称
        @params child_model: 表体模型名称
        ...
        """
        parent_data = records.query_read( 
                fields=['number', 'create_uid', 'pay_org_id', 'biz_date', 'sett_curr_id', 'local_curr_id', 'state', 'sett_org_id'])
        lines_dict = self._get_line_dict(
            doc_ids=records.ids, 
            model=child_model,
            domain=[('sett_type_id', 'in', cash_sett_type_ids)],
            fields=['sett_type_id', 'sequence', 'cash_acct_id', 'discount_amount', 'discount_amount_local', 
                'commission', 'commission_local', 'note', 'parent_id', 'pay_type_id']
            )
        result = []
        for parent in parent_data:
            ...
            parent_dict = {
                'source_create_uid': parent['create_uid'],  # 创建人#
                'number': parent['number'],  # 单据编号#
                'recpay_org_id': parent['pay_org_id'],  # 收付组织#
                'biz_date': parent['biz_date'],  # 业务日期#
                'state': parent['state'],  # 单据状态#
                'local_currency_id': parent['local_curr_id'],  # 本位币#
                'source_parent_doc_id': parent['id'],  # 来源单据主表id
                'currency_id': parent['sett_curr_id'],  # 币别#
                'sett_org_id': parent['sett_org_id'],  # 结算组织
            }
            for line in lines_dict[parent['id']]:
                base_dict = {
    ...
    def _query_cm_payable_refund_bill_line1_data(self, records, parent_model, child_model, cash_sett_type_ids):
        """
        @desc: 创建采购业务付款退款单/其他业务付款退款单相关数据到后台表中
        @params: self: 实列
        @params records: 表头对象
        ...
        lines_dict = self._get_line_dict(
            ...
            domain=[('sett_type_id', 'in', cash_sett_type_ids)],
            fields=['sett_type_id', 'sequence', 'cash_acct_id', 'discount_amount', 'discount_amount_local', 
                'commission', 'commission_local', 'pay_type_id', 'note', 'parent_id']
            )
        result = []
        for parent in parent_data:
    ...
    def _query_cm_manual_journal_line1_data(self, records, parent_model, child_model, cash_sett_type_ids):
        """
        @desc: 创建现金手工日记账相关数据到后台表中
        @params: self: 实列
        @params records: 表头对象
        ...
        """
        parent_data = records.query_read( 
                fields=['number', 'create_uid', 'payrecv_org_id', 'biz_date', 'currency_id', 'local_currency_id', 'state',])
        lines_dict = self._get_line_dict(
            doc_ids=records.ids, 
            model=child_model,
            domain = [],
        ...
        for parent in parent_data:
            ...
            parent_dict = {
                'source_create_uid': parent['create_uid'],  # 创建人#
                'number': parent['number'],  # 单据编号#
                'recpay_org_id': parent['payrecv_org_id'],  # 收付组织#
                'biz_date': parent['biz_date'],  # 业务日期#
                'state': parent['state'],  # 单据状态#
                'local_currency_id': parent['local_currency_id'],  # 本位币#
                'source_parent_doc_id': parent['id'],  # 来源单据主表id
            ...
            for line in lines_dict[parent['id']]:
                ...
                if float_is_zero(income_amount, ) != 0 or income_amount_local != 0:
                # 如果收入金额/收入金额本位币/支出金额/支出金额本位币都为 0 ，则不进行数据创建
                    new_dict = {
                        'income_amount': income_amount,  # 收入金额
                        'expense_amount': 0,  # 支出金额
    ...
    def p_get_unaudit_data(self, start_date, end_date):
        ...
        for parent_model, child_model, parent_domain in DOC_MODELS:
            ...
            func_name = '_query_%s_data' % self.env[child_model]._table
            if hasattr(self, func_name):
                result.extend(getattr(self, func_name)(records, parent_model, child_model, cash_sett_type_ids))
