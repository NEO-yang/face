...
class ErExpenseListener(Component):
    ...
    @skip_if(lambda self, source_model_key, source_doc_ids, time, amount: source_model_key != 'er.expense')
    def on_update_er_push_amount(self, source_model_key, source_doc_ids, time, amount):
        """
        付款单保存/修改/删除时，更新费用报销单费用明细已下推金额
        @param source_model_key: 来源单据模型
        @param source_doc_ids: 来源单据id列表
        @param time: reverse（反写）或者reverse_cancel（取消反写）
        @param amount: 待更新的金额
        """
        er_expense_obj = self.env['er.expense']
        records = er_expense_obj.browse(source_doc_ids)

        for record in records:
            currency_id = record.currency_id.id
            if self._is_zero(amount, currency_id):
                # 如果待更新金额=0，则跳出循环（防止后续有合单下推的情况）
                break
            if time == 'reverse':
                # 反写时，累加已下推金额
                for line in record.line1_expense_ids:
                    if not self._is_zero(amount, currency_id):
                        # 如果amount不是0，则依次更新费用明细行已下推金额
                        unpush_amount = line.unpush_amount
                        if not self._is_zero(unpush_amount, currency_id):
                            # 如果未下推付款金额不是0，则更新此费用明细行已下推金额
                            if self._float_compare(unpush_amount, amount, currency_id) <= 0:
                                # 如果未下推付款金额小于等于待更新的金额，则已下推付款金额=未下推付款金额
                                line.push_amount = self._round(line.push_amount + unpush_amount, currency_id)
                                amount = self._round(amount - unpush_amount, currency_id)
                            else:
                                # 如果未下推付款金额大于待更新的金额，则已下推付款金额=当前已下推付款金额 + 待更新的金额
                                line.push_amount = self._round(line.push_amount + amount, currency_id)
                                amount = 0
                    else:
                ...
                for line in reversed(record.line1_expense_ids):
                    if not self._is_zero(amount, currency_id):
                        # 如果amount不是0，则依次更新费用明细行已下推金额
                        push_amount = line.push_amount
                        if not self._is_zero(push_amount, currency_id):
                            # 如果已下推付款金额不是0，则更新此费用明细行已下推金额
                            if self._float_compare(push_amount, amount, currency_id) <= 0:
                                # 如果已下推付款金额小于等于待更新的金额，则已下推付款金额=0
                                line.push_amount = 0
                                amount = self._round(amount - push_amount, currency_id)
                            else:
                                # 如果已下推付款金额大于待更新的金额，则已下推付款金额=当前已下推付款金额 - 待更新的金额
                                line.push_amount = self._round(push_amount - amount, currency_id)
                                amount = 0
                    else:
                        # 如果待更新的金额（amount）变为0，则跳出循环
                        break 
    ...
    def _update_pay_amount_and_recon_vals(self, record, time, amount, recon_vals):
        ...
        settle_type_id = record.settle_type_id.id,  
        if time == 'reverse':
            ...
            for line in record.line1_expense_ids:
                if not self._is_zero(amount, currency_id):
                    ...
                    if not self._is_zero(unpay_amount, currency_id):
                        old_pay_amount_lc = line.pay_amount_lc
                        ...
                        if self._float_compare(unpay_amount, amount, currency_id) <= 0:
                            ...
                            amount = 0
                        # 本行该次核销本位币金额
                        curr_recon_amount_lc = self._round(line.pay_amount_lc - old_pay_amount_lc, local_currency_id)
                        # 更新核销记录的数据列表
                        recon_vals.append({
                            ...
                            'recon_amount_lc': curr_recon_amount_lc, # 本行该次核销本位币金额
                            'settle_type_id': settle_type_id, # 结算方式
    ...
    def _update_other_payable_sett_amount(self, record, time, amount, recon_vals):
        ...
        currency_id = record.currency_id.id
        local_currency_id = record.local_currency_id.id
        settle_type_id = record.sett_type_id.id
        if time == 'reverse':
            ...
            for line in record.line_ids:
                if not self._is_zero(amount, currency_id):
                    ...
                    if not self._is_zero(unrecon_amount, currency_id):
                        old_sett_amount_lc = line.sett_amount_lc
                        ...
                        if self._float_compare(unrecon_amount, amount, currency_id) <= 0:
                            ...
                            amount = 0
                        # 本行该次核销本位币
                        curr_recon_amount_lc = self._round(line.sett_amount_lc - old_sett_amount_lc, local_currency_id)
                        # 更新核销记录的数据列表
                        recon_vals.append({
                            'biz_date': biz_date,  # 业务日期(申请日期)
                            'biz_doc_id': biz_doc_id,  # 业务单据id
                            ...
                            'unrecon_amount': line.unrecon_amount,  # 未核销金额
                            'recon_amount_lc': curr_recon_amount_lc, # 本次核销金额本位币
                            'settle_type_id': settle_type_id, # 结算方式
                            'local_currency_id': local_currency_id, # 本位币
                        })
                else:
    ...
    def _create_recon_record(self, er_expense, other_payable, payable_bill, recon_vals):
        ...
        recon_record = recon_record_obj.create(recon_vals)
        
        # 创建其他应付调汇记录及应付款明细表
        recon_record_obj.p_create_exchange_revaluation_record(other_payable, payable_bill, recon_record)
    ...
    def _delete_recon_record(self, target_doc):
        ...
        # 删除应付调汇单记录及应付款明细表记录
        recon_record_obj.p_detele_exchange_revaluation_record(target_doc)
