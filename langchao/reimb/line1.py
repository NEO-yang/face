...
class AccCenterVoucherGeneration(models.Model):
                ...
                try:
                    ...
                    if curr_id:
                        ...
                        if amount:
                            ...
                            # 辅助核算
                            flex, auxiliary = self._get_auxiliary(data, config_data, acct_set_line)
                            # 更新凭证明细数据
    ...
    def _merge_voucher_lines(self, exist_lines, exist_mapping, pending_lines, local_currency_id):
        ...
        for line in pending_lines:
            ...
            # 更具摘要，科目，辅助核算，币别进行合并
            line_key = (line['summary'], line['account_id'], line['auxiliary'], currency_id)
    ...
    def _merge_voucher(self, origin_data, merge_fields, accting_org_id, accting_book, template_id, is_merge_voucher):
        """
        合并凭证
        ...
        for doc_id, data in origin_data.items():
            ...
            if data['fail_reason']:
                ...
                for line in line_ids:
                    account_set.add(line['account_id'])
                acct_type_count = self.env['mdm.account.account'].search_count([('id', 'in', list(account_set)), ('acct_type', '!=', None)])
                if account_set and acct_type_count and len(account_set) != acct_type_count:
                    # 更新是否为现金类凭证标志
                    # 1. 科目中必须包含现金类科目
                    # 2. 所有科目都是现金类科目, 则此凭证不视为现金类凭证
                    is_acct_type = True
                else:
                    is_acct_type = False
        ...
        for voucher_line in voucher_lines:
            if is_combine:
                ...
                if debit * credit > 0:
                    ...
                    debit, credit, debit_local, credit_local = (debit - credit, 0, debit_local - credit_local, 0) \
                        if self._float_compare(abs(debit), abs(credit), currency_id) > 0 \
                        else (0, credit - debit, 0, credit_local - debit_local)
                elif debit * credit < 0:
                    debit, credit, debit_local, credit_local = (debit - credit, 0, debit_local - credit_local, 0) \
                        if self._float_compare(debit, credit, currency_id) > 0 \
                        else (0, credit - debit, 0, credit_local - debit_local)
    ...
    def _create_voucher(self, documents, origin_data, merge_fields, accting_org_id, accting_book, template_id, is_merge_voucher):
        """
        对凭证及分录进行合并，并生产凭证
        ...
        """
        # 对单据及分录按照配置进行合并
        voucher_vals, tracking_vals = self._merge_voucher(origin_data, merge_fields, accting_org_id, accting_book, template_id, is_merge_voucher)
            ...
            if not line_ids:
                ...
                try:
                    ...
                    with self.env.cr.savepoint():
                        ...
                        total = self._round(sum([line.debit_local if line.debit_local else -line.credit_local for line in voucher.line_ids if line.account_id.acct_type in ['cash','cashequ','bank']]), self.acct_book_id.currency_id.id)
                        if not total:
                            self.env['gl.voucher'].open_cashflow_window(voucher.id)
                except ValidationError as e:
                    # 更新生成失败原因
    ...
    def p_generate_vouchers(self, lines, start_date, end_date, merge_doc_ids=None):
        """
        生成凭证
        @param lines: 凭证生成明细
        @param start_date: 开始日期
        @param end_date: 结束日期
        @param merge_doc_ids： 待合并单据ID列表
        """
        ...
        line_infos = [] # 提示信息
        voucher_ids = [] # 凭证id列表
        is_merge_voucher = False  # 是否多张单据合并生成凭证
        for line in lines:
            ...
            if merge_doc_ids:
                documents = self.env[line.source_doc_model].browse(merge_doc_ids)
                is_merge_voucher = True
            else:
            ...
            if documents:
                ...
                # 创建凭证
                infos, ids = self._create_voucher(documents, orgin_data, temp_data['merge_fields'], accting_org_id, accting_book, template_id, is_merge_voucher)
    ...
    def open_cashflow_window(self, voucher_id):
        """
        打开现金流量设置wizard
        """
        voucher = self.env['gl.voucher'].browse(voucher_id)
        is_auto_spc_cf = self._get_config(voucher.acct_book_id.id, 'is_auto_spc_cf')  # 凭证保存时自动指定现金流量
        is_required_cf = self._get_config(voucher.acct_book_id.id, 'is_required_cf')  # 凭证中现金流量科目必须录入现金流量
        if voucher.is_acct_type and not voucher.is_cash_set and voucher.state == 'save':
            if is_auto_spc_cf:
                # 凭证保存时自动指定现金流量
                cash_flow_data = []
                total = self._round(sum([line.debit_local if line.debit_local else -line.credit_local for line in voucher.line_ids if line.account_id.acct_type in ['cash','cashequ','bank']]), self.acct_book_id.currency_id.id)
                if not total:
                    # 当前凭证借贷方金额合计为零，请手工指定现金流量项目
                    raise ValidationError(_('当前凭证借贷方金额合计为零，请手工指定现金流量项目'))
                for line in voucher.line_ids:
                    if line.account_id.acct_type in ('cash','cashequ','bank'):
                        cash_flow_data.append((
                            0,0,{
                            'is_chf_acct': True,
                            'acct_book_id': voucher.acct_book_id.id,
                            'number': line.account_id.number,
                            'account_id': line.account_id.id,
                            'currency_id': line.currency_id.id,
                            'direction': 'debit' if line.debit_local else 'credit',
                            'debit_credit_local': line.debit_local if line.debit_local else line.credit_local,
                            }
                        ))
                    else:
                        if not any(line.account_id.out_chf_item_id or line.account_id.in_chf_item_id):
                            # 科目中没有设置现金流入或流出项目, 此时直接弹窗
                            return voucher._get_cashflow_wizard()
                        ...
                        cash_flow_data.append((
                            ...
                            'account_id': line.account_id.id,
                            'direction': 'debit' if line.debit_local else 'credit',
                            'chf_item_id': self._get_chf_item(total, line).id,
                            'debit_credit_local': self._get_debit_credit_local(line, self._get_chf_item(total, line)),
                            }
                        ))

                voucher.write({
                    'line_chf_ids': cash_flow_data,
                    'is_cash_set': True,
                })

                if cash_flow_data:
                    # 如果已经指定现金流量项目, 则直接返回, 无需再走凭证中现金流量科目必须录入现金流量校验
                    return
