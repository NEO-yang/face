...
class CmCashJournalQuery(models.TransientModel):
    ...
    def _get_capital_balance_data(self, domain_fields, cash_acct_number_dict):
        ...
        currencys = self.env['mdm.currency'].browse(domain_fields['currency_ids'])
        ...
        for cash_acct_number_id, cash_acct_number_data in cash_acct_number_dict.items():
            ...
            for currency in currencys:
                ...
                local_currency_id = capital_balance.local_currency_id.name or ''
                for cash_income in cash_incomes:
                    ...
                    if not local_currency_id:
                        local_currency_id = cash_income.local_currency_id.name or ''
                    if not is_to_show:
    ...
    def _get_query_period_dict(self, domain_fields, yesterday_balance_dict):
        ...
        for res in record:
            ...
            if not yesterday_balance_dict[yesterday_balance_key]['is_to_show']:
                ...
                yesterday_balance_dict[yesterday_balance_key]['local_currency_id'] = res.local_currency_id.name if res.local_currency_id else ''  # 本位币
            ...
            new_dict = {
                ...
                'currency_id': res.currency_id.name if res.currency_id else '',  # 币别
            ...
            if domain_fields['is_local_currency']:
                ...
                new_dict['local_currency_id'] = res.local_currency_id.name if res.local_currency_id else ''  # 本位币
    ...
    def _insert_yesterday_balance_data(self, domain_fields, data, query_cells, row):
        ...
        for key in ('recpay_org_id', 'acct_number_id', None, None, 'summary', 'currency_id', None, None, 'end_period_balance'):
            query_cells.append(QueryCell(row, column, data[key] if key and key in data else ''))
            column += 1
        ...
        # 收付款用途, 结算方式, 制单人, 单据状态
        for i in range(4):
            query_cells.append(QueryCell(row, column + i, ''))
        return row + 1
    ...
    def _insert_day_month_year_total_data(self, domain_fields, query_cells, row, total_dict, time_unit):
        ...
        for key in ('recpay_org_id', 'acct_number_id', time_unit + '_date', None, 'summary', 'currency_id', 
                        time_unit + '_income_total', time_unit + '_expense_total', time_unit + '_balance_total'):
        ...
        if domain_fields['is_local_currency']:
            ...
            column += 1
            query_cells.append(QueryCell(row, column, total_dict[time_unit + '_income_total_local']))  # 本位币：收入金额
            column += 1
            query_cells.append(QueryCell(row, column, total_dict[time_unit + '_expense_total_local']))  # 本位币：支出金额
            column += 1
            query_cells.append(QueryCell(row, column, total_dict[time_unit + '_balance_total_local']))  # 本位币：余额
            column += 1
        else:
            column += 4
    ...
    def _insert_query_period_data(self, domain_fields, query_period_date_dict, query_cells, row, total_dict, balance_amount,  event_data, balance_amount_local=0):
        ...
        query_cells.append(QueryCell(row, column, balance_amount))  # 原币：余额
        column += 1
        ...
        if domain_fields['is_local_currency']:
            ...
            query_cells.append(QueryCell(row, column, balance_amount_local))  # 本位币：余额
            column += 1
        else:
            column += 4
