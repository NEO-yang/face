...
class AmbExpenseDetailQuery(models.TransientModel):
    ...
    def _update_fee_sum(self, total_dict, query_detail_data_dict, total, amount, currency_id):
        ...
        for period in ['day', 'month', 'all']:
            for fee in ['exp', 'tax', 'reimb']:
                ...
                total_dict[key] = self._round(total_dict[key], currency_id)
    ...
    def _get_query_cells(self, domain_fields):
        ...
        @return: 返回表格数据，跳转动作
        """
        lines_data, parent_id_dict, state_dict = self._get_record_data(domain_fields)
        ...
        for line in lines_data:
            ...
            line_dict = self._get_lines_dict(line, line_parent_dict, state_dict, dept_dict)  # 组装明细行与表头数据成字典
            if not domain_fields['is_total']:
                ...
                self._insert_query_data(domain_fields, line_dict, query_cells, row, event_data)
                row += ROW_STEP
                self._update_total_dict(domain_fields, line_dict, total_dict)
                if  line == lines_data[LAST_DATA_INDEX]:
